...
function App() {
	...
	const [people2020, setPeople2020] = useState({});
	const [people2019, setPeople2019] = useState({});
	const [people2014, setPeople2014] = useState({});
	const [people2003, setPeople2003] = useState({});
	const [speed, setSpeed] = useState("slow");
	const [showMore, setShowMore] = useState(true);
	// const [time, setTime] = useState(0);
	const [groups2021, setGroups2021] = useState({ ...defaultGroups });
	const [groups2020, setGroups2020] = useState({ ...defaultGroups });
	const [groups2019, setGroups2019] = useState({ ...defaultGroups });
	const [groups2014, setGroups2014] = useState({ ...defaultGroups });
	const [groups2003, setGroups2003] = useState({ ...defaultGroups });
	const [nodes2021, setNodes2021] = useState([]);
	const [nodes2020, setNodes2020] = useState([]);
	const [nodes2019, setNodes2019] = useState([]);
	const [nodes2014, setNodes2014] = useState([]);
	const [nodes2003, setNodes2003] = useState([]);
	...
	function timer() {
		...
		nodes2020.forEach(function (o, i) {
			// Increment time.
			let tempTime = time.current + 1;
			...
			if (tempTime >= 1439) {
				...
				if (nextGroup !== o.group) {
					groups2020[o.group] = { ...groups2020[o.group], cnt: groups2020[o.group].cnt - 1 };
		...
		nodes2019.forEach(function (o, i) {
			// Increment time.
			let tempTime = time.current + 1;
			...
			if (tempTime >= 1439) {
				...
				if (nextGroup !== o.group) {
					groups2019[o.group] = { ...groups2019[o.group], cnt: groups2019[o.group].cnt - 1 };
		...
		nodes2014.forEach(function (o, i) {
			// Increment time.
			let tempTime = time.current + 1;
			...
			if (tempTime >= 1439) {
				...
				if (nextGroup !== o.group) {
					groups2014[o.group] = { ...groups2014[o.group], cnt: groups2014[o.group].cnt - 1 };
		...
		nodes2003.forEach(function (o, i) {
			// Increment time.
			let tempTime = time.current + 1;
			...
			if (tempTime >= 1439) {
				...
				if (nextGroup !== o.group) {
					groups2003[o.group] = { ...groups2003[o.group], cnt: groups2003[o.group].cnt - 1 };
		...
		select("#timecount2 .cnt").text(formatTime(time.current));
		...
		select("#chart2020")
			.selectAll(".group-count")
			.text((d) => getCountPercentage(groups2020[d].cnt));

		// Update counters.
		select("#chart2019")
			.selectAll(".group-count")
			.text((d) => getCountPercentage(groups2019[d].cnt));

		// Update counters.
		select("#chart2014")
			.selectAll(".group-count")
			.text((d) => getCountPercentage(groups2014[d].cnt));

		// Update counters.
		select("#chart2003")
			.selectAll(".group-count")
			.text((d) => getCountPercentage(groups2003[d].cnt));
	...
	useEffect(() => {
		...
		select("#chart2").style("width", width + margin.left + margin.right + "px");

		const svg3 = select("#chart2019")
			.append("svg")
			.attr("width", width + margin.left + margin.right)
			.attr("height", height + margin.top + margin.bottom)
			.attr("viewBox", [0, 0, width * 1.2, height])
		...
		select("#chart3").style("width", width + margin.left + margin.right + "px");

		const svg4 = select("#chart2014")
			.append("svg")
			.attr("width", width + margin.left + margin.right)
			.attr("height", height + margin.top + margin.bottom)
			.attr("viewBox", [0, 0, width * 1.2, height])
		...
		// // Load data.
		const stages2019 = csv("data/processed_data_2019.csv", autoType);
		const stages2020 = csv("data/processed_data_2020.csv", autoType);
		const stages2021 = csv("data/processed_data_2021.csv", autoType);
		const stages2014 = csv("data/processed_data_2014.csv", autoType);
		const stages2003 = csv("data/processed_data_2003.csv", autoType);
		...
		stages2020.then((data) => {
			const tempPeople = {};
			const tempGroups = { ...groups2020 };
			// console.log(tempGroups);
			// Group data by pid.
			// The data file is one row per person
			data.forEach((d) => {
			...
			setPeople2020(tempPeople);
			setGroups2020(tempGroups);
			setNodes2020(nodes);
			// Start things off after a few seconds.
			// d3.timeout(timer, 5000);
		});

		stages2019.then((data) => {
			const tempPeople = {};
			const tempGroups = { ...groups2019 };
			// console.log(tempGroups);
			// Group data by pid.
			// The data file is one row per person
			data.forEach((d) => {
			...
			setPeople2019(tempPeople);
			setGroups2019(tempGroups);
			setNodes2019(nodes);
			// Start things off after a few seconds.
			// d3.timeout(timer, 5000);
		});

		stages2014.then((data) => {
			const tempPeople = {};
			const tempGroups = { ...groups2014 };
			// console.log(tempGroups);
			// Group data by pid.
			// The data file is one row per person
			data.forEach((d) => {
			...
			setPeople2014(tempPeople);
			setGroups2014(tempGroups);
			setNodes2014(nodes);
			// Start things off after a few seconds.
			// d3.timeout(timer, 5000);
		});

		stages2003.then((data) => {
			const tempPeople = {};
			const tempGroups = { ...groups2003 };
			// console.log(tempGroups);
			// Group data by pid.
			// The data file is one row per person
			data.forEach((d) => {
			...
			setPeople2003(tempPeople);
			setGroups2003(tempGroups);
			setNodes2003(nodes);
			// Start things off after a few seconds.
			// d3.timeout(timer, 5000);
		});
	...
	return (
		<div className='App'>
			...
			{showMore && (
				<Box className='simulation-container' sx={{ mt: -19 }}>
					<Box
						sx={{
							display: "flex",
							flexDirection: "column",
					...
					<Box
						...
						<Box
							...
							<Typography id='timecount2' sx={{ fontSize: "2.5rem", textAlign: "center", mb: 2 }}>
								Time of day:{" "}
								<span className='cnt'>{time.current ? formatTime(time.current) : "4:00am"}</span>
							</Typography>
							<ButtonGroup
								variant='text'
								size='small'
